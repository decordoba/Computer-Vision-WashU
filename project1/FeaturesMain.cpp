...
int mainComputeFeatures(int argc, char **argv) {
    ...
    printf("Features computed and saved to %s\n", argv[3]);
...
int mainMatchFeatures(int argc, char **argv) {
    ...
    printf("%d matches for threshold %.3f\n", num_good_matches, threshold);
    fprintf(f, "%d\n", num_good_matches);
    for (int i = 0; i < num_matches; i++) {
        if (matches[i].score < threshold) {
    ...
    printf("Features from %s and %s matched and saved to %s\n", argv[2], argv[3], matchFile);
